...
_add('bisect-angle', r'Create a line that bisects the angle evenly in two, then press submit\.', [])
...
_add('book-flight', r'Book the (.*) one-way flight from: (.*) to: (.*) on (.*)\.', ['criterion', 'from', 'to', 'date'])
...
# Select 12/10/2016 as the date and hit submit.
_add('choose-date', r'Select 0*(\d*)/0*(\d*)/0*(\d*) as the date and hit submit\.', ['month', 'day', 'year'])
FIELD_EXTRACTORS['choose-date-nodelay'] = \
        FIELD_EXTRACTORS['choose-date-easy'] = \
...
_add('circle-center', r'Find and click on the center of the circle, then press submit\.', [])
...
_add('click-collapsible-2', r'Expand the sections below, to find and click on the link "(.*)"\.', ['target'])
FIELD_EXTRACTORS['click-collapsible-2-nodelay'] = FIELD_EXTRACTORS['click-collapsible-2']
...
# Click the button in the dialog box labeled "Cancel".
# Click the button in the dialog box labeled "Cancel".
_add('click-dialog-2', r'Click the button in the dialog box labeled "(.*)"\.', ['target'])
...
# Click the "Menu" button, and then find and click on the item with the icon.
# TODO
_add('click-menu-2', r'Click the "Menu" button, and then find and click on the item (.*)\.', ['target'])
...
_add('click-pie', r'Expand the pie menu below and click on the item labeled "(.*)"\.', ['target'])
...
_add('click-scroll-list', r'Select (.*) from the scroll list and click Submit\.', ['target'])
...
_add('click-tab-2', r'Switch between the tabs to find and click on the link "(.*)"\.', ['target'])
FIELD_EXTRACTORS['click-tab-2-easy'] = FIELD_EXTRACTORS['click-tab-2-medium'] = FIELD_EXTRACTORS['click-tab-2-hard'] = FIELD_EXTRACTORS['click-tab-2']
...
_add('copy-paste', r'Copy the text in the textarea below, paste it into the textbox and press Submit\.', [])
...
# Copy the text from the 2nd text area below and paste it into the text input, then press Submit.
# Copy the text from the 2nd text area below and paste it into the text input, then press Submit.
_add('copy-paste-2', r'Copy the text from the (\d+).. text area below and paste it into the text input, then press Submit\.', ['target'])
...
_add('count-sides', r'Press the button that correctly denotes how many sides the shape has\.', [])

_add('drag-box', r'Drag the smaller box so that it is completely inside the larger box\.', [])
...
# Move the cube around so that "2" is the active side facing the user.
# Move the cube around so that "3" is the active side facing the user.
_add('drag-cube', r'Move the cube around so that "(.*)" is the active side facing the user\.', ['target'])
...
_add('drag-sort-numbers', r'Sort the numbers in increasing order, starting with the lowest number at the top of the list\.', [])
...
FIELD_EXTRACTORS['email-inbox-star-reply'] = \
        FIELD_EXTRACTORS['email-inbox'] = \
...
def extract_email_inbox_nl(utterance):
    return Fields({})
FIELD_EXTRACTORS['email-inbox-forward-nl'] = \
        FIELD_EXTRACTORS['email-inbox-forward-nl-turk'] = \
        FIELD_EXTRACTORS['email-inbox-nl-turk'] = \
...
_add('enter-password', r'Enter the password "(.*)" into both text fields and press submit\.', ['target'])
...
# Type "JOYE" in all lower case letters in the text input and press Submit.
_add('enter-text-2', r'Type "(.*)" in all (.*) case letters in the text input and press Submit\.', ['text', 'case'])
...
_add('enter-text-dynamic', r'Enter "(.*)" into the text field and press Submit\.', ['target'])
...
_add('find-midpoint', r'Find and click on the shortest mid-point between the two points, then press submit\.', [])
...
_add('find-word', r'Find the (\d+).. word in the paragraph, type that into the textbox and press "Submit"\.', ['target'])
...
_add('guess-number', r'Guess the number between 0-9 and press Submit\. Use the feedback below to find the right number\.', [])

_add('highlight-text', r'Highlight the text in the paragraph below and click submit\.', [])
...
# Highlight the text in the 3rd paragraph and click submit.
# Highlight the text in the 3rd paragraph and click submit.
_add('highlight-text-2', r'Highlight the text in the (\d+).. paragraph and click submit\.', ['target'])
...
_add('login-user', r'Enter the username "(.*)" and the password "(.*)" into the text fields and press login\.', ['username', 'password'])
...
_add('multi-layouts', r'Search for (.*) movies directed by (.*) from year (.*)\.', ['genre', 'director', 'year'])
...
_add('navigate-tree', r'Navigate through the file tree\. Find and click on the folder or file named "(.*)"\.', ['target'])
...
# Draw the number "9" in the checkboxes using the example on the right and press Submit when finished.
_add('number-checkboxes', r'Draw the number "(.*)" in the checkboxes using the example on the right and press Submit when finished\.', ['target'])
...
# Enter the value of Country into the text field and press Submit.
# Enter the value of Religion into the text field and press Submit.
_add('read-table', r'Enter the value of (.*) into the text field and press Submit\.', ['target'])

_add('read-table-2', r'Enter the value that corresponds with each label into the form and submit when done\.', [])
...
# Resize the textarea so that the width is smaller than its initial size then press Submit.
# Resize the textarea so that the height is larger than its initial size then press Submit.
_add('resize-textarea', r'Resize the textarea so that the (.*) than its initial size then press Submit\.', ['target'])

_add('right-angle', r'Add a third point to create a right angle, then press submit\.', [])

_add('scroll-text', r'Find the last word in the text area, enter it into the text field and hit Submit\.', [])
...
# Scroll the textarea to the top of the text hit submit.
# Scroll the textarea to the bottom of the text hit submit.
_add('scroll-text-2', r'Scroll the textarea to the (.*) of the text hit submit\.', ['target'])
...
_add('search-engine', r'Use the textbox to enter "(.*)" and press "Search", then find and click the (\d+).. search result\.', ['query', 'rank'])

_add('simon-says', r'Push the buttons in the order displayed\.', [])

_add('simple-algebra', r'Solve for x and type your answer into the textbox\. Press Submit when done\.', [])

_add('simple-arithmetic', r'Solve the math problem and type your answer into the textbox\. Press submit when done\.', [])
...
_add('social-media', r'For the user (.*), click on the "(.*)" button\.', ['user', 'button'])
...
_add('social-media-all', r'Click the "(.*)" button on all posts by (.*) and then click (Submit)\.', ['button', 'user', 'submit'])
...
_add('social-media-some', r'Click the "(.*)" button on (.*) posts? by (.*) and then click (Submit)\.', ['button', 'amount', 'user', 'submit'])
...
_add('text-transform', r'Type the text below into the text field and press Submit\.', [])
...
def extract_use_autocomplete(utterance):
    match = re.match(r'Enter an item that starts with "([^"]*)" and ends with "([^"]*)"\.', utterance)
    if match:
...
# Select white with the color picker and hit Submit.
_add('use-colorwheel', r'Select (.*) with the color picker and hit Submit\.', ['target'])

_add('use-colorwheel-2', r'Select the following color with the color picker and hit Submit\.', [])
...
_add('use-slider-2', r'Set the sliders to the combination \[(.*),(.*),(.*)\] and submit\.', ['n1', 'n2', 'n3'])
...
_add('visual-addition', r'Type the total number of blocks into the textbox and press Submit\.', [])
